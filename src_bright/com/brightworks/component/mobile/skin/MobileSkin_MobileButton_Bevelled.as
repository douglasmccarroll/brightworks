...
public class MobileSkin_MobileButton_Bevelled extends MobileSkin {
   private static const _FILL_COLOR:uint = 0x5F6CC1;
   ...
   public function MobileSkin_MobileButton_Bevelled() {
      super();
   ...
   override protected function createChildren():void {
      ...
      // dmccarroll 20161231
      // For some inexplicable reason I need to keep this in order for my background to render.
      // If it's not on the display list the background doesn't render.
      // If its alpha is set to zero the background doesn't render.
      // If its text is set to "" the background doesn't render.
      // So I'm keeping it and setting its alpha to .01.
      // The 'correct' fix would be to upgrade Button_Standard so that it includes toggle button func'y
   ...
   override protected function layoutContents(unscaledWidth:Number, unscaledHeight:Number):void {
      ...
      //
      // This fix may be caused by the fact that, since I changed labelDisplay from a
   ...
   private function drawBackgroundRect():void {
      super.drawBackground(unscaledWidth, unscaledHeight);
      var brightnessMultiplier:Number;
      switch (currentState) {
         case "down":
         ...
         case "disabled": {
            brightnessMultiplier = .8;
            break;
         }
         default: {
            Log.error("MobileSkin_MobileButton_Bevelled.drawBackground(): No case for current state of '" + currentState + "' - using default");
            brightnessMultiplier = .25;
         }
      }
      Utils_Graphic.drawBevelledSurface(
            ...
            Utils_Button.computeBevelButtonStandardBevelThickness(this),
            Utils_Button.computeBevelButtonStandardBevelThickness(this));
   }
